  GNU nano 6.2                   MostafaAssign1.py                            
import subprocess
import sys
import re
...
def get_packet_loss(ip):
    try:
        ...
        output = subprocess.check_output(["ping", "-c", "4", ip], universal_n>
...
def main(ips):
    for ip in ips:
        ...
        if loss is not None:
            print(f"For the IP address {ip}, the rate of packet loss is: {los>
        else:
            print(f"For the IP address {ip}, the Destination Host/Net is Unre>
